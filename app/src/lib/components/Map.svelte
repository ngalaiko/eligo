<script lang="ts">
	...
	const map = (container: HTMLElement, items: Item[]) => {
		...
		const marker = async ({ title, coordinates }: Item) => {
			...
			const markerIcon = () =>
				L.divIcon({
					html: `<svg xmlns="http://www.w3.org/2000/svg" class="fill-green" width="15" height="15" viewBox="0 0 24 24" stroke-width="1.5" stroke="#2c3e50" stroke-linecap="round" stroke-linejoin="round">
